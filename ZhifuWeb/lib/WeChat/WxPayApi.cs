...
namespace ZhifuWeb.lib.WeChat
{
    public class WxPayApi
    {
        ...
        /// 统一下单
        /// </summary>
        /// <param name="wxPayData">提交给统一下单API的参数</param>
        /// <param name="timeOut">超时时间</param>
        /// <returns>成功时返回，其他抛异常</returns>
        public static WxPayData UnifiedOrder(WxPayData wxPayData, int timeOut = 6)
        {
            ...
            else if (!wxPayData.IsSet("body"))
            {
                throw new WxPayException("缺少统一支付接口必填参数body！");
            }
            else if (!wxPayData.IsSet("total_fee"))
            {
                throw new WxPayException("缺少统一支付接口必填参数total_fee！");
            }
            else if (!wxPayData.IsSet("trade_type"))
            {
                throw new WxPayException("缺少统一支付接口必填参数trade_type！");
            }

            //关联参数
            if (wxPayData.GetValue("trade_type").ToString() == "JSAPI" && !wxPayData.IsSet("openid"))
            {
                throw new WxPayException("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");
            }
            if (wxPayData.GetValue("trade_type").ToString() == "NATIVE" && !wxPayData.IsSet("product_id"))
            {
                throw new WxPayException("统一支付接口中，缺少必填参数product_id！trade_type为JSAPI时，product_id为必填参数！");
            }

            //异步通知url未设置，则使用配置文件中的url
            if (!wxPayData.IsSet("notify_url"))
            {
                var notifyurl = "http://" + HttpContext.Current.Request.Url.Authority + WxPayConfig.NOTIFY_URL;
                wxPayData.SetValue("notify_url", notifyurl);//异步通知url
            }

            wxPayData.SetValue("appid", WxPayConfig.APPID);//公众账号ID
            wxPayData.SetValue("mch_id", WxPayConfig.MCHID);//商户号
            wxPayData.SetValue("spbill_create_ip", WxPayConfig.IP);//终端ip
            wxPayData.SetValue("nonce_str", GenerateNonceStr());//随机字符串

            LogHelper.CreateLog(string.Format("WxPayApi.UnifiedOrder:{0}", wxPayData.ToJson()), "wxdebug");
            //签名
            ...
            LogHelper.CreateLog(string.Format("WxPayApi.UnifiedOrder   UnfiedOrder request:{0}", xml), "wxdebug");
            string response = HttpService.Post(xml, url, false, timeOut);
            LogHelper.CreateLog(string.Format("WxPayApi.UnifiedOrder   UnfiedOrder response:{0}", xml), "wxdebug");
